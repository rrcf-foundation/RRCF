"""
converters/common.py — format-agnostic RRCF 1.0 draft builder.

Every format-specific analyzer (mjcf.py, urdf.py, sdf.py, ...) reduces its
source file down to the same plain-dict shape:

    {
        "has_freejoint": bool,        # floating/mobile base present?
        "buckets": {                  # DOF names bucketed by function
            "leg": [...], "arm": [...], "hand": [...],
            "wheel": [...], "other": [...],
        },
        "actuators": [                # one entry per actuated DOF
            {"name": str, "ctrlrange": (lo, hi), "bucket": str}, ...
        ],
        "sensors": [str, ...],        # sensor/telemetry field names, if any
        "nu": int,                    # actuator/DOF count
        "njnt": int,                  # joint count
    }

build_rrcf() below turns that dict into an RRCF 1.0 XML tree. Adding a new
source format means writing one analyze_*(path) function that returns this
...
def build_rrcf(info, category, meta_name, vendor, source_path, source_format, override=None):
    """source_format is one of 'mjcf' | 'urdf' | 'sdf' | 'usd' — used for the
    <physical_ref format="..."/> tag so a controller knows which physics file
    to load alongside this .rrcf."""
    ET.register_namespace("", RRCF_NS)
    root = ET.Element("rrcf", version="1.0", xmlns=RRCF_NS)
    root.append(ET.Comment(safe_comment(
        f" AUTO-GENERATED DRAFT via {source_format}_to_rrcf — verify before use. "
        f"{info['nu']} actuators / {info['njnt']} joints in source {source_format.upper()}. "
    ...
    ET.SubElement(meta, "physical_ref", format=source_format, path=str(source_path))
    ...
    skills.append(ET.Comment(
        ...
        "spec section 9 (VLA Integration) for why. "
    ))
    ...
    if info["sensors"]:
        ...
        telemetry.append(ET.Comment(
            f" No sensors found in source {source_format.upper()} — battery field is a placeholder. "
        ))

    safety = ET.SubElement(root, "safety")
    slug = meta_name.lower().replace(" ", "_")
    ET.SubElement(safety, "estop", required="true", topic=f"/rrcf/{slug}/estop", qos="2")
